...
function brandColor(m){return m?BRAND_COLORS[m.toLowerCase().trim()]||"#1976D2":"#1976D2";}
function makeInitials(m){if(!m)return"?";const w=m.trim().split(/\s+/);return w.length>=2?(w[0][0]+w[1][0]).toUpperCase():m.slice(0,2).toUpperCase();}
function logoHtml(make,size=20){const c=brandColor(make),i=makeInitials(make),f=size<=16?Math.round(size*0.5):Math.round(size*0.42);return`<div style="width:${size}px;height:${size}px;border-radius:${size<=20?"50%":"8px"};background:${c};display:flex;align-items:center;justify-content:center;font-size:${f}px;font-weight:700;color:#fff;flex-shrink:0;letter-spacing:-0.5px">${i}</div>`;}

const SVC_LABELS={oil:"Oelwechsel",inspection:"Inspektion",brake_fluid:"Bremsfl\u00FCssigkeit",cabin_filter:"Innenraumfilter",air_filter:"Luftfilter",spark_plugs:"Z\u00FCndkerzen",fuel_filter:"Kraftstofffilter",gearbox:"Getriebe\u00F6l",haldex:"Haldex-\u00D6l",ac:"Klimawartung",hu:"Hauptuntersuchung (HU/AU)"};
const SVC_ICONS={oil:"mdi:oil",inspection:"mdi:clipboard-check-outline",brake_fluid:"mdi:car-brake-alert",cabin_filter:"mdi:fan",air_filter:"mdi:air-filter",spark_plugs:"mdi:lightning-bolt",fuel_filter:"mdi:gas-station",gearbox:"mdi:cog-transfer",haldex:"mdi:car-4wd",ac:"mdi:air-conditioner",hu:"mdi:car-search"};
const REP_LABELS={brakes_front:"Bremse vorne",brakes_rear:"Bremse hinten",brakes_full:"Bremsen komplett",discs_front:"Bremsscheiben vorne",discs_rear:"Bremsscheiben hinten",shock_front:"Sto\u00DFd\u00E4mpfer vorne",shock_rear:"Sto\u00DFd\u00E4mpfer hinten",timing_belt:"Zahnriemen",battery:"Batterie",clutch:"Kupplung",other:"Sonstiges"};
const TIRE_WARN={summer:3.0,winter:4.0,allseason:4.0};
const TIRE_MIN=1.6;
const WEAR=1.5/10000;

function calcPct(v,sid){
  const last=(v.lastService||{})[sid]||{},intv=(v.intervals||{})[sid]||{},ez=v.ezDate,curKm=v.km||0;
  let kp=null,kl=null,tp=null,ml=null;
  if(intv.km){const base=last.km!=null?last.km:0,driven=curKm-base;kp=Math.min(100,Math.round(driven/intv.km*100));kl=Math.max(0,intv.km-driven);}
  if(intv.months){const bd=last.date||ez||null;if(bd){const ms=(Date.now()-new Date(bd).getTime())/(1000*60*60*24*30.44);tp=Math.min(100,Math.round(ms/intv.months*100));ml=Math.max(0,Math.round(intv.months-ms));}else{tp=0;ml=intv.months;}}
...
const PROF_STEPS=[];for(let pv=0;pv<=12;pv+=0.5)PROF_STEPS.push(parseFloat(pv.toFixed(1)));
function sel(id,opts,val=""){return`<select id="${id}">${opts.map(o=>`<option value="${o.v||o}"${(o.v||o)==val?"selected":""}>${o.l||o}</option>`).join("")}</select>`;}
function inp(id,type,ph,val="",extra=""){return`<input id="${id}" type="${type}" placeholder="${ph}" value="${val}" ${extra}/>`;}
...
class VehicleServiceCard extends HTMLElement {
  ...
  async _ws(msg){try{await this._hass.callWS(msg);await this._load();}catch(e){alert("Fehler: "+(e.message||e));await this._load();}}
  ...
  _vhdr(){const v=this._v();return`<div class="vhdr">${logoHtml(v.make||"",34)}<div><div class="vtit">${v.make||""} ${v.model||""}</div><div class="vmeta">${v.plate?`<span class="chip">${v.plate}`:""}</span>${v.ezDate?`<span class="chip">EZ ${fd(v.ezDate)}</span>`:""}${v.entity?`<span class="chip live">\u2B24 Live KM</span>`:""}</div></div></div>`;}
  ...
  _metrics(){
    ...
    return`<div class="mets"><div class="met" style="cursor:pointer" id="met-km" title="KM aktualisieren"><div class="ml">Kilometerstand \u270E</div><div class="mv">${fkm(v.km)}</div></div><div class="met"><div class="ml" style="color:#3B6D11">\u2713 OK</div><div class="mv" style="color:#3B6D11">${ok}</div></div><div class="met"><div class="ml" style="color:#BA7517">\u26A1 Bald</div><div class="mv" style="color:#BA7517">${warn}</div></div><div class="met"><div class="ml" style="color:#A32D2D">\u26A0 F\u00E4llig</div><div class="mv" style="color:#A32D2D">${crit}</div></div></div>`;
  }

  _tabbar(){const tabs=[{id:"status",l:"Service Status"},{id:"history",l:"Service Historie"},{id:"repairs",l:"Reparaturen"},{id:"tires",l:"Reifen"}];return`<div class="tabbar">${tabs.map(t=>`<button class="tab${this._tab===t.id?" on":""}" data-tab="${t.id}">${t.l}</button>`).join("")}</div>`;}
  ...
  _status(v){
    ...
    return`<div class="tc">${(v.services||[]).map(sid=>{
      ...
      const lastStr=last.date?`${fd(last.date)}${last.km?" \u00B7 "+fkm(last.km):""}`:v.ezDate?`Kein Eintrag \u2013 Basis: EZ ${fd(v.ezDate)}`:"Kein Eintrag";
      const statusLbl=r.tier==="red"?(r.pct>=100?"\u00DCberf\u00E4llig":"F\u00E4llig bald"):r.tier==="yellow"?"Im Blick":"OK";
      const parts=[];if(r.kl!==null)parts.push(r.kl<=0?"km \u00FCberf\u00E4llig":fkm(r.kl));if(r.ml!==null)parts.push(r.ml<=0?"Zeit abgelaufen":r.ml+" Mon.");
      return`<div class="srow"><div class="sico" style="background:${bg};color:${col}"><ha-icon icon="${SVC_ICONS[sid]||"mdi:wrench"}"></ha-icon></div><div class="sbod"><div class="snm">${SVC_LABELS[sid]||sid}</div><div class="slt">${lastStr}</div><div class="pbar"><div class="pfil" style="width:${r.pct}%;background:${col}"></div></div></div><div class="srt"><span class="badge" style="background:${bg};color:${col}">${statusLbl}</span><div class="ssub">${parts.join(" \u00B7 ")}</div></div></div>`;
    }).join("")}</div>`;
  }
  ...
  _history(v){
    ...
    const rows=hist.map((h,i)=>{
      const chips=(h.services||[]).map(sid=>`<span class="chip2"><ha-icon icon="${SVC_ICONS[sid]||"mdi:wrench"}" style="--mdc-icon-size:11px"></ha-icon>${SVC_LABELS[sid]||sid}</span>`).join("");
      const editBtn=h.auto?"":`<button class="ibtn edit-svc" data-idx="${i}">\u270E</button>`;
      return`<div class="hrow"><div class="hd">${fd(h.date)}${h.auto?`<span class="atag">auto</span>`:""}<div class="hkm">${fkm(h.km)}</div></div><div class="hb">${h.notes?`<div class="hn">${h.notes}</div>`:""}<div class="chrow">${chips}</div></div><div style="display:flex;gap:2px">${editBtn}<button class="ibtn del-svc" data-idx="${i}">\u2715</button></div></div>`;
    }).join("");
    return`<div class="tc"><div class="shdr"><span>Eintr\u00E4ge</span><button class="addbtn" id="btn-svc">+ Eintrag</button></div>${rows||`<div class="empty">Noch keine Eintr\u00E4ge.</div>`}</div>`;
  }

  _repairs(v){
    const reps=[...(v.repairs||[])].sort((a,b)=>new Date(b.date)-new Date(a.date));
    const rows=reps.map((r,i)=>`<div class="rrow"><div class="rico"><ha-icon icon="mdi:wrench"></ha-icon></div><div class="rbod"><div class="rnm">${REP_LABELS[r.cat]||r.cat}</div>${r.desc?`<div class="rdsc">${r.desc}</div>`:""}<div class="rmt">${fd(r.date)}${r.km?" \u00B7 "+fkm(r.km):""}</div></div>${r.cost?`<div class="rco">${parseFloat(r.cost).toLocaleString("de-DE",{minimumFractionDigits:0})} \u20AC</div>`:""}<button class="ibtn del-rep" data-idx="${i}">\u2715</button></div>`).join("");
    return`<div class="tc"><div class="shdr"><span>Reparaturen</span><button class="addbtn" id="btn-rep">+ Hinzuf\u00FCgen</button></div>${rows||`<div class="empty">Noch keine Reparaturen.</div>`}</div>`;
  }
  ...
  _tires(v){
    ...
    if(latest){
      ...
      const oc=worst<=TIRE_MIN?"#A32D2D":worst<=wm?"#BA7517":"#3B6D11",ol=worst<=TIRE_MIN?"Kritisch!":worst<=wm?"Grenzwertig":"OK";
      const wheels=["vl","vr","hl","hr"].map((pos,i)=>{
        ...
        const col=worn<=TIRE_MIN?"#A32D2D":worn<=wm?"#BA7517":"#3B6D11",lbl=worn<=TIRE_MIN?"Kritisch":worn<=wm?"Grenzwertig":"OK";
        const pct=Math.min(100,Math.max(0,(worn/orig)*100)),kml=Math.round(Math.max(0,(worn-wm)/WEAR));
        return`<div class="tw"><div class="twp">${["VL","VR","HL","HR"][i]}</div><div class="twv" style="color:${col}">${worn} mm</div><div class="twbar"><div style="width:${pct}%;background:${col};height:100%;border-radius:2px"></div></div><div class="twl" style="color:${col}">${lbl}</div>${worn>wm?`<div class="twkm">~${kml.toLocaleString("de-DE")} km</div>`:""}</div>`;
      }).join("");
      currentHtml=`<div class="tire-status-header"><div class="tire-type-badge" style="background:${oc}20;color:${oc};border:1px solid ${oc}40">${tLbl}</div><div style="color:${oc};font-size:12px;font-weight:500">\u25CF ${ol}</div></div><div class="tire-info-row">${sz?`<span class="ms mono">${sz}</span>`:""} ${latest.brand?`<span class="ms">${latest.brand}</span>`:""}</div><div class="tgrid">${wheels}</div><div class="tnote">Verschlei\u00DF: 1,5 mm / 10.000 km \u00B7 Empf. Grenze: ${wm} mm \u00B7 Min.: 1,6 mm</div>`;
    }
    const histRows=history.map((t,i)=>{const tt=t.type||"summer";const tLbl={summer:"Sommerreifen",winter:"Winterreifen",allseason:"Ganzjahresreifen"}[tt]||tt;const sz=t.width&&t.ratio&&t.rim?`${t.width}/${t.ratio} R${t.rim}`:"";return`<div class="hrow" style="grid-template-columns:100px 1fr auto"><div class="hd">${fd(t.date)}<div class="hkm">${fkm(t.km)}</div></div><div class="hb"><div style="font-size:12px;font-weight:500">${tLbl}${sz?` \u00B7 <span style="font-family:monospace">${sz}</span>`:""}</div>${t.brand?`<div style="font-size:11px;color:var(--secondary-text-color)">${t.brand}</div>`:""}</div><button class="ibtn del-tire" data-idx="${i}">\u2715</button></div>`;}).join("");
    return`<div class="tc"><div class="shdr"><span>Aktuell montiert</span><button class="addbtn" id="btn-tire">+ Reifen eintragen</button></div><div class="tcard">${currentHtml}</div>${history.length>1?`<div class="shdr" style="margin-top:14px"><span>Reifenhistorie</span></div>${histRows}`:""}</div>`;
  }

  _showSvcModal(){const v=this._v();const svcs=(v.services||[]).map(sid=>`<label class="cblabel"><input type="checkbox" class="svc-cb" value="${sid}"> ${SVC_LABELS[sid]||sid}</label>`).join("");this._modal=`<div class="overlay" id="modal"><div class="mbox"><div class="mhdr">Service-Eintrag <button class="closebtn" id="mclose">\u2715</button></div><div class="mrow2">${inp("m-date","date","",today())} ${inp("m-km","number","KM",v.km||"")}</div><div class="mfld"><label>Arbeiten</label><div class="cbgrid">${svcs}</div></div><div class="mfld"><label>Notizen</label>${inp("m-notes","text","Werkstatt...")}</div><div class="mbtn-row"><button class="sbtn" id="m-save-svc">Speichern</button></div></div></div>`;this._paint();}
  _showRepModal(){const v=this._v();const catOpts=Object.entries(REP_LABELS).map(([k,l])=>({v:k,l}));this._modal=`<div class="overlay" id="modal"><div class="mbox"><div class="mhdr">Reparatur <button class="closebtn" id="mclose">\u2715</button></div><div class="mrow2">${inp("r-date","date","",today())} ${inp("r-km","number","KM",v.km||"")}</div><div class="mfld"><label>Kategorie</label>${sel("r-cat",catOpts)}</div><div class="mfld"><label>Beschreibung</label>${inp("r-desc","text","z.B. Bremsbel\u00E4ge")}</div><div class="mfld"><label>Kosten \u20AC</label>${inp("r-cost","number","0")}</div><div class="mbtn-row"><button class="sbtn" id="m-save-rep">Speichern</button></div></div></div>`;this._paint();}
  _showTireModal(){const v=this._v();const wOpts=WIDTHS.map(w=>({v:w,l:w})),rOpts=RATIOS.map(r=>({v:r,l:r})),rimOpts=RIMS.map(r=>({v:r,l:r})),pOpts=PROF_STEPS.map(p=>({v:p,l:p.toFixed(1)+" mm"}));this._modal=`<div class="overlay" id="modal"><div class="mbox"><div class="mhdr">Reifen eintragen <button class="closebtn" id="mclose">\u2715</button></div><div class="mrow2">${inp("t-date","date","",today())} ${inp("t-km","number","KM",v.km||"")}</div><div class="mrow2"><div class="mfld"><label>Typ</label>${sel("t-type",[{v:"summer",l:"Sommerreifen"},{v:"winter",l:"Winterreifen"},{v:"allseason",l:"Ganzjahr"}])}</div><div class="mfld"><label>Achse</label>${sel("t-axle",[{v:"all",l:"Alle vier"},{v:"front",l:"VA"},{v:"rear",l:"HA"}])}</div></div><div class="mfld"><label>Gr\u00F6\u00DFe</label><div class="sizerow">${sel("t-w",wOpts,"205")}<span class="sep">/</span>${sel("t-r",rOpts,"55")}<span class="sep">R</span>${sel("t-rim",rimOpts,"16")}</div><div class="sizeprev" id="sp">\u2192 205/55 R16</div></div><div class="mrow2"><div class="mfld"><label>Marke</label>${inp("t-brand","text","Michelin")}</div><div class="mfld"><label>DOT</label>${inp("t-dot","text","2323","","maxlength='4'")}<div class="dotprev" id="dp"></div></div></div><div class="mfld"><label>Profiltiefe</label><div class="profgrid"><div><label class="plbl">VL</label>${sel("t-vl",pOpts,"8.0")}</div><div><label class="plbl">VR</label>${sel("t-vr",pOpts,"8.0")}</div><div><label class="plbl">HL</label>${sel("t-hl",pOpts,"8.0")}</div><div><label class="plbl">HR</label>${sel("t-hr",pOpts,"8.0")}</div></div></div><div class="mbtn-row"><button class="sbtn" id="m-save-tire">Speichern</button></div></div></div>`;this._paint();}
  _showKmModal(){const v=this._v();this._modal=`<div class="overlay" id="modal"><div class="mbox" style="max-width:340px"><div class="mhdr">KM aktualisieren <button class="closebtn" id="mclose">\u2715</button></div><div class="mfld"><label>Kilometerstand</label><input id="km-val" type="number" value="${v.km||0}" style="width:100%;padding:10px;border-radius:8px;border:1px solid var(--divider-color);background:var(--secondary-background-color);color:var(--primary-text-color);font-size:18px;font-weight:500;box-sizing:border-box;text-align:right"/></div><div class="mbtn-row"><button class="sbtn" id="m-save-km">Speichern</button></div></div></div>`;this._paint();}
  _showEditSvcModal(hist,idx){const v=this._v(),h=hist[idx];const svcs=(v.services||[]).map(sid=>`<label class="cblabel"><input type="checkbox" class="svc-cb" value="${sid}" ${(h.services||[]).includes(sid)?"checked":""}> ${SVC_LABELS[sid]||sid}</label>`).join("");this._editIdx=idx;this._editHist=hist;this._modal=`<div class="overlay" id="modal"><div class="mbox"><div class="mhdr">Eintrag bearbeiten <button class="closebtn" id="mclose">\u2715</button></div><div class="mrow2"><input id="m-date" type="date" value="${h.date}"/><input id="m-km" type="number" value="${h.km||0}"/></div><div class="mfld"><label>Arbeiten</label><div class="cbgrid">${svcs}</div></div><div class="mfld"><label>Notizen</label><input id="m-notes" type="text" value="${h.notes||""}"/></div><div class="mbtn-row"><button class="sbtn" id="m-save-edit-svc">Aktualisieren</button></div></div></div>`;this._paint();}
  ...
  _bind(){
    ...
    s.querySelectorAll(".del-svc").forEach(b=>b.addEventListener("click",async()=>{if(!confirm("Eintrag l\u00F6schen?"))return;const v=this._v(),vid=this._vid();const hist=[...(v.history||[])].sort((a,b2)=>new Date(b2.date)-new Date(a.date));const h=hist[parseInt(b.dataset.idx)];const ri=(v.history||[]).findIndex(x=>x.date===h.date&&x.km===h.km&&JSON.stringify(x.services)===JSON.stringify(h.services));await this._ws({type:`${DOMAIN}/delete_service_entry`,vehicle_id:vid,entry_index:ri});}));
    s.querySelectorAll(".del-rep").forEach(b=>b.addEventListener("click",async()=>{if(!confirm("L\u00F6schen?"))return;const v=this._v(),vid=this._vid();const reps=[...(v.repairs||[])].sort((a,b2)=>new Date(b2.date)-new Date(a.date));const r=reps[parseInt(b.dataset.idx)];const ri=(v.repairs||[]).findIndex(x=>x.date===r.date&&x.cat===r.cat);await this._ws({type:`${DOMAIN}/delete_repair`,vehicle_id:vid,repair_index:ri});}));
    s.querySelectorAll(".del-tire").forEach(b=>b.addEventListener("click",async()=>{if(!confirm("L\u00F6schen?"))return;const v=this._v(),vid=this._vid();const ti=[...(v.tires||[])].sort((a,b2)=>new Date(b2.date)-new Date(a.date));const t=ti[parseInt(b.dataset.idx)];const ri=(v.tires||[]).findIndex(x=>x.date===t.date&&x.km===t.km);await this._ws({type:`${DOMAIN}/delete_tire`,vehicle_id:vid,tire_index:ri});}));
    ...
    s.getElementById("m-save-svc")?.addEventListener("click",async()=>{const vid=this._vid(),date=s.getElementById("m-date").value,km=parseInt(s.getElementById("m-km").value)||0,notes=s.getElementById("m-notes").value,services=[...s.querySelectorAll(".svc-cb:checked")].map(x=>x.value);if(!date){alert("Bitte Datum eingeben.");return;}if(!services.length){alert("Bitte einen Service-Punkt w\u00E4hlen.");return;}this._closeModal();await this._ws({type:`${DOMAIN}/add_service_entry`,vehicle_id:vid,entry_date:date,km,services,notes});});
    s.getElementById("m-save-rep")?.addEventListener("click",async()=>{const vid=this._vid(),date=s.getElementById("r-date").value,km=parseInt(s.getElementById("r-km").value)||0,category=s.getElementById("r-cat").value,description=s.getElementById("r-desc").value,cost=parseFloat(s.getElementById("r-cost").value)||0;if(!date){alert("Bitte Datum eingeben.");return;}this._closeModal();await this._ws({type:`${DOMAIN}/add_repair`,vehicle_id:vid,entry_date:date,km,category,description,cost});});
    s.getElementById("m-save-tire")?.addEventListener("click",async()=>{const vid=this._vid(),date=s.getElementById("t-date").value,km=parseInt(s.getElementById("t-km").value)||0,tire_type=s.getElementById("t-type").value,axle=s.getElementById("t-axle").value,width=parseInt(s.getElementById("t-w").value),ratio=parseInt(s.getElementById("t-r").value),rim=parseInt(s.getElementById("t-rim").value),brand=s.getElementById("t-brand").value,dot=s.getElementById("t-dot").value.replace(/\D/g,"").slice(0,4),vl=parseFloat(s.getElementById("t-vl").value)||0,vr=parseFloat(s.getElementById("t-vr").value)||0,hl=parseFloat(s.getElementById("t-hl").value)||0,hr=parseFloat(s.getElementById("t-hr").value)||0;if(!date){alert("Bitte Datum eingeben.");return;}this._closeModal();await this._ws({type:`${DOMAIN}/add_tire`,vehicle_id:vid,entry_date:date,km,tire_type,axle,width,ratio,rim,brand,dot,vl,vr,hl,hr});});
    s.getElementById("m-save-edit-svc")?.addEventListener("click",async()=>{const vid=this._vid(),v=this._v(),date=s.getElementById("m-date").value,km=parseInt(s.getElementById("m-km").value)||0,notes=s.getElementById("m-notes").value,services=[...s.querySelectorAll(".svc-cb:checked")].map(x=>x.value);if(!date||!services.length)return;const h=this._editHist[this._editIdx];const ri=(v.history||[]).findIndex(x=>x.date===h.date&&x.km===h.km&&JSON.stringify(x.services)===JSON.stringify(h.services));this._closeModal();await this._ws({type:`${DOMAIN}/update_service_entry`,vehicle_id:vid,entry_index:ri,entry_date:date,km,services,notes});});
    ...
    s.getElementById("t-dot")?.addEventListener("input",e=>{const v=e.target.value.replace(/\D/g,"").slice(0,4);e.target.value=v;const el=s.getElementById("dp");if(el)el.textContent=v.length===4?`KW ${v.slice(0,2)} / 20${v.slice(2,4)}`:"";}); }
...
class VehicleServiceCompactCard extends HTMLElement {
  ...
  _main(){
    const v=this._v();
    const pills=this._vehicles.length>1?`<div class="cpills">${this._vehicles.map((vv,i)=>`<button class="cpill${i===this._cur?" on":""}" data-ci="${i}">${logoHtml(vv.make||"",14)}</button>`).join("")}</div>`:"";
    const icons=(v.services||[]).map(sid=>{const r=calcPct(v,sid),col=TIER_COL[r.tier],bg=TIER_BG[r.tier];return`<div class="iico" style="background:${bg};color:${col}" title="${SVC_LABELS[sid]||sid}"><ha-icon icon="${SVC_ICONS[sid]||"mdi:wrench"}"></ha-icon></div>`;}).join("");
    let tireIco="";
    const tires=v.tires||[];
    if(tires.length){const lat=tires[tires.length-1],tt=lat.type||"summer",wm=TIRE_WARN[tt]||3,mKm=parseInt(lat.km)||0;let worst=999;["vl","vr","hl","hr"].forEach(pos=>{const orig=parseFloat(lat[pos])||0;if(orig){const worn=Math.max(0,orig-Math.max(0,(v.km||0)-mKm)*WEAR);if(worn<worst)worst=worn;}});const col=worst<=TIRE_MIN?"#A32D2D":worst<=wm?"#BA7517":"#3B6D11",bg=worst<=TIRE_MIN?"#FCEBEB":worst<=wm?"#FAEEDA":"#EAF3DE";tireIco=`<div class="iico" style="background:${bg};color:${col}" title="Reifen"><ha-icon icon="mdi:car-tire-alert"></ha-icon></div>`;}
    return`${pills}<div class="chdr">${logoHtml(v.make||"",28)}<div class="chdr-text"><div class="cvtit">${v.make||""} ${v.model||""}</div><div class="cvkm">${fkm(v.km)}</div></div></div><div class="igrid">${icons}${tireIco}</div>`;
  }
